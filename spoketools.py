...
def get_peaks(datapoints,row,plot=None):
    
    rows, cols = datapoints.shape
	
    profile = np.array(datapoints[row,:])
	
    x = np.arange(cols) # x as in x-axis
    ...
    if plot == 1:
        ...
        ax.plot(x,profile)
        # ax.plot(peaks,profile[peaks],"x")
        ax.set_xlabel('Row {}'.format(row))
...
#	getd=mean-sigma #for dark spokes
    getd = mean+sigma #for light spokes
    m,n=datapoints.shape
...
def remove_cosmic_rays(pixel_values):
    ...
    for i in range(0, m):
        top20 = np.sort(pixel_values[i])[-20:]
